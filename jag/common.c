...
volatile void **
jag_get_cache_sets(cjag_config_t *config) { /// selects ths eviction-sets
    ...
    uint8_t *addr = mmap(NULL, n_pages * 2 * 1024 * 1024, PROT_READ | PROT_WRITE,   /// allocates the 2mb pages
    ...
    for (int p = 1; p < n_pages; ++p) { /// inserts all potential lines (S_p,i,j) to test_eviction_set
    ...
    for (int p = n_pages - 1; p > 0; --p) {
        ...
        slice_offset[p] = config->cache_slices - 1; /// will hold the slice number that evicts the probe-set
        for (int i = 0; i < config->cache_slices; ++i) {
            ...
            if (slice_offset[p] < 0) {  /// case no slice has lines that evicts the probe-set
                return NULL;
            }
        }
    ...
    return cache_set_out; /// holds all the eviction-sets(containing their lines)
}


uint32_t
jag_check_set(volatile uint8_t **s_addrs, uint32_t target_misses, uint32_t read_timeout, cjag_config_t *config) {   /// primes and probes
...
uint32_t
jag_check_set2(volatile uint8_t **s_addrs, uint32_t target_misses, uint32_t read_timeout, cjag_config_t *config) {   /// primes and probes
    uint8_t hit[500], miss[500];
    uint32_t hits = 0, misses = 0, reads = 0;
    size_t time, delta;
    int window_index = 0, max_misses = 0;
