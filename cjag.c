...
int main(int argc, char **argv) {
    ...
    if (config.send) {
        ...
        watchdog_reset(config.watchdog);
        watchdog_done(config.watchdog);
        config.jag_send_count = 5000 * 3;
        config.jag_recv_count = 15000 * 3;
        jag_sendFile(&config, send_callback);
        ...
        config.jag_send_count = 5000 * 3;
        config.jag_recv_count = 15000 * 3;
        jag_receiveBit(addrs,&config, receive_callback);
...
void show_welcome(cjag_config_t *config) {
    printf_color(config->color_output,
                 "\n[y]_______[/y]______________[r]Start[/r]______________[y]_______[/y]\n");
}
